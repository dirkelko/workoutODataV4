sap.ui.define([
	...
	let Timer = Control.extend("fsworkouts.control.Timer", {
		metadata : {
			properties : {
		...
		startClock : function (start){
			...
			interval = setInterval(function(){
				...
				timerDom.querySelector("#exerciseName").innerHTML = (bPause)? pauseText : aExercises[intervalIndex].exercise.name;
				...
				if (remTime < 5000 && bPause ){
					...
					timerDom.querySelector("#exerciseName").innerHTML = aExercises[intervalIndex].exercise.name;
				}
				if (remTime <= 0 && intervalIndex >= aExercises.length -1 && !bPause){
					...
					that.fireEvent("exerciseFinished", {
						exerciseID: aExercises[intervalIndex].ID
					});
			} else if (remTime <= 0 && intervalIndex < aExercises.length){
					if (bPause){
						that.fireEvent("exerciseStarted", {
							exerciseID: aExercises[intervalIndex].ID
						});
								//bellSingle.play();
					...
					`Exercise ${aExercises[intervalIndex].exRound}/${aExercises.filter(ex=>{ return ex.round == aExercises[intervalIndex].round}).length}`;
		...
		nextExercise : function(){
			...
			timerDom.querySelector("#exerciseName").innerHTML = exercises[intervalIndex].exercise.name;
			timerDom.querySelector("#exerciseInfo").innerHTML = 
			`Exercise ${exercises[intervalIndex].exRound}/${exercises.filter(ex=>{ return ex.round == exercises[intervalIndex].round}).length}`;
			timerDom.querySelector("#roundInfo").innerHTML = 
